...
def display_column_info(csv_file):
    try:
        ...
        df.columns = [col.replace(" ", "_").replace("-", "_").replace(".", "_").replace("#", "_").replace("/", "_") for col in df.columns] 
...
if __name__ == "__main__":
    ...
    sdlc_parser.add_argument("--csvFile", "-csv", help="Specify CSV file to convert and upload to Snowflake")
    ...
    if not os.path.isfile(args.csvFile):
        print("CSV file does not exist. Specify a valid/correct CSV file.")
        sys.exit()

    df, original_data_types = display_column_info(args.csvFile)
